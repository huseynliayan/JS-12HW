let goods = [
  ...
  const showList = (arr) => {
    arr.forEach((item) => {
      ...
      p1.innerText = item.product_name;
      p1.id = item.product_name;
      p2.innerText = item.product_description;
      p3.innerText = item.product_price;
      p4.innerText = item.store_name;
      p5.innerText = item.store_address;
      li.appendChild(p1);
      li.appendChild(p2);
      li.appendChild(p3);
      li.appendChild(p4);
  ...
  const filterWithPrice = () => {
    ...
    let secondRange = prompt();
let filteredArray = goods.filter((item) => item.product_price >= parseInt(firstRange) && item.product_price <= parseInt(secondRange));
    ul.innerHTML = "";
  ...
  const addNewProduct = () => {
    let newProduct = {
        product_name: prompt("Введите название продукта:"),
        product_description: prompt("Введите описание продукта:"),
        product_price: parseFloat(prompt("Введите цену продукта:")),
        store_name: prompt("Введите название магазина:"),
        store_address: prompt("Введите адрес магазина:")
    };
